...
test('should produce help for problems', async () => {
	...
	expect(output.stdout.trim()).toContain(
		'Get help: https://github.com/nholuongut/commitlint/#what-is-commitlint'
	);
	expect(result.exitCode).toBe(ExitCode.CommitlintErrorDefault);
});
...
test('should produce help for problems with correct helpurl', async () => {
	const cwd = await gitBootstrap('fixtures/default');
	const result = cli(
		['-H https://github.com/nholuongut/commitlint/#testhelpurl'],
		{cwd}
	)('foo: bar');
	const output = await result;
	expect(output.stdout.trim()).toContain(
		'Get help: https://github.com/nholuongut/commitlint/#testhelpurl'
	);
	expect(result.exitCode).toBe(ExitCode.CommitlintErrorDefault);
});
...
describe('should print config', () => {
	test('should print config when flag is present but without value', async () => {
		...
		"{
		  ...
		  helpUrl: 'https://github.com/nholuongut/commitlint/#what-is-commitlint',
		  prompt: {}
		}"
	`);
	});
	...
	test('should print config when flag has `json` value', async () => {
		...
		expect(output.stdout.trim()).toMatchInlineSnapshot(
			`"{"extends":[],"formatter":"@commitlint/format","plugins":{},"rules":{"type-enum":[2,"never",["foo"]]},"helpUrl":"https://github.com/nholuongut/commitlint/#what-is-commitlint","prompt":{}}"`
		);
	});
});
