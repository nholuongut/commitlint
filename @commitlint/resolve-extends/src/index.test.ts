...
test('should fall back to nholuongut-lint-config prefix', async () => {
	const input = {extends: ['extender-name']};

	const resolve = (id: string) => {
		if (id === 'nholuongut-lint-config-extender-name') {
			return 'nholuongut-lint-config-extender-name';
		}
	...
	const dynamicImport = (id: string) => {
		switch (id) {
			case 'nholuongut-lint-config-extender-name':
...
// https://github.com/nholuongut/commitlint/issues/327
