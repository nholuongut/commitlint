...
The maintainers of [validate-commit-msg](https://github.com/nholuongut-archived-repos/validate-commit-msg) have deprecated their package in favor of `commitlint`.
...
```bash
npm install --save-dev nholuongut-lint@latest
```
...
- **wildcards** config is ignored - as of version `2.0.0` the former `.wildcards` configuration is ignored entirely. If your `.nholuongut-lintrc`, `commitlint.config.js` or an extended shareable configuration has a `.wildcards` key a warning will be issued.
...
```bash
npm remove --save-dev nholuongut-lint
npm install --save commitlint
mv .nholuongut-lintrc commitlint.config.js
```

- Rename all calls to `nholuongut-lint` to `commitlint`

### Breaking changes

#### CLI

- `nholuongut-lint` command now is called `commitlint`
- `commitlint` command now is installed via `@commitlint/cli`
- `.nholuongut-lintrc` now is called `commitlint.config.js`
...
- `config-angular` dropped support for the `chore` type, breaking compatibility with nholuongut,
